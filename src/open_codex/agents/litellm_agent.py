...
class LiteLLMAgent(LLMAgent):
    ...
    def _check_litellm_available(self) -> None:
        ...
        try:
            ...
            logger.error(f"Could not connect to LiteLLM server at ~/your_opencodex_config/path/.env.")
            raise ConnectionError(f"Could not connect to LiteLLM server at ~/your_opencodex_config/path/.env: {str(e)}")
...
Return as JSON array:"""
        ...
        try:
            ...
            for idx, step_data in enumerate(steps_data, start=1):
                step = TaskStep(
                    id=step_data.get("id", f"step{idx}"),
    ...
    def execute_step(self, step: TaskStep, context: TaskContext) -> str:
        ...
        # 生成命令
        raw = self._generate_completion(messages).strip()
        command = self._extract_command(raw)
        if not command:
            # 强制重试：严格只返回代码块命令
            messages[-1]["content"] += "\nIMPORTANT: You must return only a single bash fenced code block with the command."
            raw = self._generate_completion(messages).strip()
            command = self._extract_command(raw) or raw.splitlines()[0]
        
        try:
            # DRY-RUN 支持
            if context.metadata and context.metadata.get('dry_run'):
                return f"DRY-RUN:\n命令: {command}"
            # 实际执行命令
    ...
    def generate_step_prompt(self, step: TaskStep, context: TaskContext) -> str:
        """
        Generate a strict prompt to produce an executable shell command for the step.
        """
        prev_ok = [s.description for s in context.steps[:context.current_step_index] if s.status == StepStatus.COMPLETED]
        dep_ids = ", ".join([str(d) for d in (step.dependencies or [])])
        instructions = (
            "You are a command generator. Return ONE executable POSIX shell command that completes the step. "
            "Output MUST be a single fenced code block with language 'bash' and only the command inside. "
            "No explanations, no comments, no multiple lines unless strictly necessary via '&&'. "
            "Respect dependency order: add apt keys before repositories, update indexes before install. "
        )
        prompt = (
            f"{instructions}\n"
            f"Task: {context.user_prompt}\n"
            f"Step ID: {step.id}\n"
            f"Step: {step.description}\n"
            f"Dependencies: {dep_ids}\n"
            f"Previous completed: {prev_ok[-3:]}\n"
            "Return as:\n```bash\n<command>\n```"
        )
        return prompt
...
Please suggest how to recover from this error or if the step should be skipped."""
        ...
        try:
            ...
            return None
    # custome call logging 
    # def fingo_litemml_logging_fn(model_call_dict):
    ...
    def _generate_completion(self, messages: List[Dict[str, str]]) -> str:
        ...
        try:
            response = litellm.completion(
                ...
                messages=messages,
                # seens mostly provides were suport by litellm check https://docs.litellm.ai/docs/providers/zai
            ...
            # just in response debug case
            print("debug litellm raw response:", response)
            ...
            # debug case 
            print("debug exception during litellm.completion:", e)
            raise ConnectionError(f"Error communicating with LiteLLM: {str(e)}")
            
    def _extract_command(self, text: str) -> Optional[str]:
        """Extract a single shell command from LLM output focusing on fenced code blocks."""
        import re
        m = re.search(r"```(?:bash|sh)?\n([\s\S]*?)\n```", text)
        if m:
            cmd = m.group(1).strip()
            return cmd.splitlines()[0] if "\n" in cmd else cmd
